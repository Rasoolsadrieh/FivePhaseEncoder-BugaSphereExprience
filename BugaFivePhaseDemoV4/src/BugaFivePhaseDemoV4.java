...
public class BugaFivePhaseDemoV4 {
    ...
    enum BreathMode {
        INHALE_6, // Inhale 6s (phases 0,1,2), Exhale 4s (phases 3,4)
        INHALE_8  // Inhale 8s (phases 0,1,2,3), Exhale 2s (phase 4)
    }

    /* ---------- UI panel: color + smoothly rotating pentagon + labels ---------- */
    static class Panel extends JPanel {
        ...
        boolean showHud = true;     // toggle with H

        // Breath overlay
        boolean showBreatheWords = true; // always on per user request
        String breathWord = "INHALE";    // INHALE / EXHALE
        BreathMode mode = BreathMode.INHALE_6;

        // compute smooth rotation for this phase
        ...
        // timer drives the rotation smoothly (~60 FPS)
        ...
        void setBreathWord(String w){ breathWord = w; repaint(); }
        void setBreathMode(BreathMode m){ mode = m; repaint(); }
        ...
        @Override protected void paintComponent(Graphics g){
            ...
            // Rotating pentagon (fill + outline)
            ...
            if (showBreatheWords) {
                ...
                Color overlay = new Color(0,0,0,90);
                if (contrast(cur.color) == Color.BLACK) overlay = new Color(0,0,0,120);
                else overlay = new Color(255,255,255,140);
                g2.setColor(overlay);
            ...
            if (showHud) {
                ...
                String modeStr = (mode==BreathMode.INHALE_6) ? "Mode: INHALE 6s / EXHALE 4s" : "Mode: INHALE 8s / EXHALE 2s";
                String hint = "6 = 6/4   8 = 8/2   F11 = Fullscreen   SPACE = Pause   ←/→ = Prev/Next   H = Toggle HUD   ESC = Quit";
                g2.setFont(getFont().deriveFont(Font.PLAIN, Math.max(15f, getWidth()*0.018f)));
                g2.drawString(modeStr, 20, getHeight()-44);
                g2.drawString(hint, 20, getHeight()-20);
            }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            JFrame f = new JFrame("Five-Phase Encoder — V4 (Breath 6/4 or 8/2)");
            ...
            f.addKeyListener(new KeyAdapter() {
                @Override public void keyPressed(KeyEvent e) {
                    switch (e.getKeyCode()) {
                        ...
                        case KeyEvent.VK_H:      panel.showHud = !panel.showHud; panel.repaint(); break;
                        case KeyEvent.VK_6:      mode[0] = BreathMode.INHALE_6; panel.setBreathMode(mode[0]); break;
                        case KeyEvent.VK_8:      mode[0] = BreathMode.INHALE_8; panel.setBreathMode(mode[0]); break;
                    }
                }
            });
            ...
            Thread loop = new Thread(() -> {
                ...
                try {
                    ...
                    while (f.isDisplayable()) {
                        ...
                        // decide INHALE / EXHALE word for this phase & current mode
    ...
    // True if the current phase index is part of INHALE for the selected mode.
    // Phases: 0=Origin,1=Growth,2=Peak,3=Decline,4=Renewal
    private static boolean isInhale(BreathMode mode, int phaseIndex){
        switch (mode) {
            case INHALE_6: // Inhale: 0,1,2  (6s). Exhale: 3,4  (4s)
                return (phaseIndex == 0 || phaseIndex == 1 || phaseIndex == 2);
            case INHALE_8: // Inhale: 0,1,2,3 (8s). Exhale: 4  (2s)
                return (phaseIndex == 0 || phaseIndex == 1 || phaseIndex == 2 || phaseIndex == 3);
            default:
                return true;
        }
    }
