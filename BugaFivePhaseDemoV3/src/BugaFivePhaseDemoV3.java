...
public class BugaFivePhaseDemoV3 {
    ...
    static class Panel extends JPanel {
        ...
        // Rotation: animate continuously with a Swing Timer (60 FPS-ish)
        double rotationDeg = 0;          // current angle
        double targetAddDeg = 72;        // how much to add over one phase
        double perFrameDeg = 0;          // computed per phase = targetAddDeg / framesInPhase

        boolean showHud = true;          // toggle with H

        // call at the start of each phase to compute smooth rotation
        ...
        // timer drives the rotation smoothly
        final Timer anim = new Timer(16, e -> {
            rotationDeg += perFrameDeg;   // small increment each frame
            repaint();
        });
        ...
        @Override protected void paintComponent(Graphics g){
            ...
            // Rotating pentagon (draw filled semi-transparent + outline for visibility)
            ...
            if (showHud) {
                ...
                String hint = "F11 = Fullscreen   SPACE = Pause/Resume   ←/→ = Prev/Next   H = Toggle HUD   ESC = Quit";
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            JFrame f = new JFrame("Five-Phase Encoder — V3");
            ...
            Thread loop = new Thread(() -> {
                ...
                try {
                    ...
                    while (f.isDisplayable()) {
                        ...
                        // set current phase + compute smooth rotation speed for this phase
                        int phaseMs = PHASES[cur].ms;
                        SwingUtilities.invokeAndWait(() -> panel.setPhase(PHASES[cur], phaseMs));

                        // clear any old interrupt and play
                        interrupt.set(false);
                        player.playPhase(PHASES[cur].hz, phaseMs, panLeft[0], paused, interrupt);
